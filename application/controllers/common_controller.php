...
//        debug($data['sql'],1);
    ...
    {
        $post = $this->input->post();
        if(isset($post['pandid']))
        {
            foreach ($post['pandid'] as $p)
            {
                $this->db->where('payment_approval_note_details_id',$p);
                $this->db->update('payment_approval_note_details',array('payment_approval_note_details_status'=>63,'approve_by'=>  $this->session->userdata('USER_ID')));
            }
        }
    ...
    public function payable_payment_create()
    {
        ...
        foreach ($post['account_amount'] as $key=>$val)
        {
            $pand_data = array(
                ...
                "payment_approval_note_details_status"=>62
            );
