...
//            
//                $inserdata = array('purchase_id'=>$post['order_id']);
//                foreach ($post['userid'] as $user_id)
//                {
//                    $inserdata['user_id'] = $user_id;
//                    $this->purchase_model->save_data($inserdata, 'purchase_approval_persons');
//                }
//                echo TRUE;
//            }
//            else
...
//            }
        ...
        $this->db->where("ref_no",$code->purchase_code);
        $this->db->delete("delegation_by_ref");
        
       
        //$hiedrarchy_query = $this->deligation_model->heirarchy_query(1,$code->purchase_code);
        //debug($hiedrarchy_query,1);
        
        $ld = $this->db->query("SELECT limit_defination FROM delegation_hierarchy WHERE approve_for_id=1")->row();
        $sql = $ld->limit_defination;
        $ld_result = $this->db->query($sql)->row();
        //return $this->db->query("SELECT manage_by,user_id,max_limit,limit_type,sort_number,step_number,must_approve,approve_priority,step_name, $ld_result->ld_amount AS limit_Amount,'$code' AS ref_no FROM delegation_hierarchy")->result_array();
        
        
        
        $this->db->query("INSERT INTO delegation_by_ref SELECT '$code->purchase_code',user_id,manage_by,$ld_result->ld_amount,max_limit,limit_type,sort_number,step_number,must_approve,approve_priority,step_name,'".date("Y-m-d")."', ".$this->session->userdata("USER_ID")." FROM delegation_hierarchy");
        //$hiedrarchy_query[]['ref_no'] = 1;
//        debug($hiedrarchy_query,1);
...
//                'approve_priority'=>$v->approve_priority,
//                'step_name'=>$v->step_name,
//                'created_by'=>  $this->session->userdata("USER_ID")
//            );
//            $this->db->insert("po_delegation",$insert_array);
//        }
    ...
    public function local_order_details_pdf_generate($info)
          {
        //$this->render_page('purchase/local_order_details_pdf_generate');
       // $this->load->view("purchase/local_order_details_pdf_generate") ;
