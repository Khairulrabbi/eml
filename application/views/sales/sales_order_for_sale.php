                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="btn-toolbar pull-right" style="padding-right: 15px;">
                            <?php
                                if($this->sales_model->sales_status_position_after_sales_order_proved($order_id)->sales_status == 24)
                                {
                                    echo '<input id="sale" class="btn btn-primary btn-sm add_item" value="Sale">';
                                }
                            ?>
...
$(document).on('click','#sale',function(){
    var sales_order_id = $('.main_order_id').val();
    $.ajax({
        url: '<?php echo base_url(); ?>sales/sales_order_sale_submit',
        type: 'POST',
        data: $('#my_form').serialize(),
        success: function (data) {
            var str = data.replace(/\n|\r/g, "");
            if (str == "done") {
                window.location.href = "<?php echo base_url(); ?>sales/all_sales_order/"+sales_order_id;
            }
        }
    });
});
