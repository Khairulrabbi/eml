...
class Master extends Custom_Controller {
    ...
    function get_html($data_master_form, $master_entry_table_title, $id = NULL,$menu_id=null, $msg = NULL){
        ...
        $get_sql_and_where_filter = $this->db->query("SELECT table_sql,where_clause_filter,export_pdf,export_excel,export_csv,enable_printing,import_excel,disable_grid_view,disable_form_view FROM master_entry_table WHERE master_entry_title='$master_entry_table_title'")->row();
        $form = array();
        $table_field = array();
        $label_list = array();
        $search_param = array();
            ...
            if($rec2['hide_from_input'] !=1 ){
                ...
                switch ($type){
                    case 'select':
                        ...
                        $search_html .= ($rec2['search'] == 1?form_dropdown($field_name.($is_join==NULL?"":"[]"), $options,$selected_value,'class="form-control"'.$validation.($is_join==NULL?"":" multiple")).'</div>':'');
                        break;
                    case 'var':
                        ...
                        $search_html .= ($rec2['search'] == 1?"$slabel_name<input type='text' name='$field_name' placeholder='$placeholder' value='$selected_value' class='form-control'/></div>":'');
                        break;
                    case 'int':
                        if($field_key == 'MUL'){
                    ...
                    default :
                        ...
                        $search_html .= ($rec2['search'] == 1?"$slabel_name<input type='text' name='$field_name' placeholder='$placeholder' value='$selected_value' class='form-control'/>":'');
                        break;
                }
    ...
    function create_master_table(){
        if($this->is_post()){
            ...
            $data['msg'] = '<div class="alert alert-success">Table created successfully.</div>';
