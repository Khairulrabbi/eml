...
function common_in_combo($name, $sql, $where, $selected_value, $extra, $id_field, $value_field, $onchange = false, $others = array()) {
    //debug($id_field,1);
    $CI = & get_instance();
    $condtion = '';
    if ($where != NULL) {
    ...
    // prepare token for concat
    if( isset($others['concat_option']) ){
        $tokens = tokenize($others['concat_option']);
    }
//    echo "<pre>";
//    print_r($tokens);
//    echo "</pre>";exit();
    foreach ($query->result_array() as $value) {
        if( isset($others['concat_option']) ){ // If option value need to concat
            //$concat_option = $others['concat_option'];
            
            $option_value_here = $others['concat_option'];
            foreach ( $tokens as $token ){
                $option_value_here = str_replace( $token, $value[$token], $option_value_here );
            }
            $data[$value[$id_field]] = $option_value_here;
//            echo $data[$value[$id_field]];exit();
        }else{
...
if(!function_exists('tokenize')){
    function tokenize( $concat_option ){
        $keywords = preg_split("/[-\'\/~`\!@#\$%\^&\*\(\)-\+=\{\}\[\]\|;:\<\>,\.\?\\\]/", $concat_option);
//        echo "<pre>";
//        print_r($keywords);
//        echo "</pre>";exit();
...
{
    function ap_drop_down( $id, $where = NULL, $others = array() ){
        $CI = & get_instance();
        ...
        // To pass for Drop Down
        $other_config = array();
        
        // check if any others configuration available. 
        $selected_value = NULL;
        $extra_attr = array();
        $name = NULL;
                
        if(!empty($others)){
            if( isset($others['selected_value']) ){
                $selected_value = $others['selected_value'];
            }
            if( isset($others['extra_attr']) ){
                $extra_attr = $others['extra_attr'];
            }
            if( isset($others['name']) ){
                $name = $others['name'];
            }            
        }else{
            $others['concat_option'] = $drop_down->concat_item;
        }
...
//        print_r($drop_down);
        ...
        return common_in_combo($name, $sql, $where, $selected_value, $extra_attr, $id_field, $value_field, FALSE, $others);
    }
}
...
function card_type($selected_value=NULL,$extra_attr=array(), $name='card_type_id',$where=NULL ){
    ...
    function account_type( $selected_value=NULL, $extra_attr=array(), $name='AccountType', $where=NULL){
    $sql = "SELECT acc_type_id,acc_type_name FROM acc_type order by acc_type_id asc";
    $id_field = 'acc_type_name';
    $value_field = 'acc_type_name';
    return common_in_combo($name,$sql,$where,$selected_value,$extra_attr,$id_field,$value_field);
}


function currency_without_id($selected_value=NULL,$extra_attr=array(), $name='currency',$where=NULL ){
