...
//        exit;
	...
	/*
     * Edite by Rokib Hasnat 18-5-2016
     * Join Purchase Type Table
     * And Select Purchase Type name
     */
    public function get_order_info($order_id){
            $this->db->select("purchase_order.*,status.status_name,shipping_method.shipping_method_name,currency.currency_name,purchase_type.purchase_type_name,purchase_type.purchase_type_id");
            ...
            $this->db->join("purchase_type",'purchase_type.purchase_type_id=purchase_order.purchase_type_id',"left");
            $this->db->where("purchase_id",$order_id,FALSE);
            $result= $this->db->get();
            //$ret = $query->row();
            return $result->row();
    }
    
    public function get_payable_payment_list($proforma_invoice_details){
            $this->db->select("payment_approval_note.payment_approval_code,cost_component.cost_component_name,payment_approval_note_details.amount,status.status_name");
            $this->db->from("payment_approval_note");
            $this->db->join("payment_approval_note_details",'payment_approval_note_details.payment_approval_note_id=payment_approval_note.payment_approval_note_id',"left");
            $this->db->join("cost_component",'cost_component.cost_component_id=payment_approval_note_details.cost_component_id','left');
            $this->db->join("status",'status.status_id=payment_approval_note.payment_approval_note_status',"left");
            $this->db->where("payment_approval_note.ref_name","proforma");
            $this->db->where("payment_approval_note.ref_name_id",$proforma_invoice_details);
            $result= $this->db->get();
...
//    public function get_waiting_approval_list($u_id){
//        $this->db->select("purchase_order.*");
//            $this->db->from("delegation_by_ref");
//            $this->db->join("purchase_order",'purchase_order.current_delegation_step=delegation_by_ref.step_number AND delegation_by_ref.ref_no =purchase_order.purchase_code',"inner");
...
//    public function get_pdf_information($Proforma_invoice_id,$field,$table){
//            $this->db->select("SELECT * FROM product");
//            $this->db->join("proforma_invoice",'proforma_invoice.product_id=product.product_id','left');
//            $this->db->where("proforma_invoice.proforma_invoice_id",$Proforma_invoice_id);
//            $result= $this->db->get();
//            return $result->result_array();
//
//    }
...
//     public function get_status($proforma_invoice_id){
//         
//          $this->db->select("s.status_name");
//          $this->db->from("status s");
//          $this->db->join("purchase_order_details pod",'pod.receive_status=s.status_id','left');
//          $this->db->where("pod.proforma_invoice_id ",$proforma_invoice_id);
//          $result= $this->db->get();
//          
//          return $result->row();
//    }
     ...
     */
	 /*
     * Edit By Rokib Hasnat 18-5-2016
     * Added Status Id in selected 
     */
    
//    public function get_all_purchase_search_history($where){
//         
//        $sql=  $this->db->query("select purchase_order.purchase_id,
...
//                                            `status`.status_id,
//                                            `status`.status_name from purchase_order
//                                            
//LEFT JOIN ON vendor purchase_order.vendor_id = vendor.vendor_id
//LEFT JOIN ON status purchase_order.vendor_id = vendor.vendor_id
//LEFT JOIN purchase_type purchase_order.purchase_type_id=purchase_type.purchase_type_id
//WHERE 
//
//");			   
//                    $this->db->from("purchase_order");
...
//                    $this->db->join("purchase_type",'purchase_order.purchase_type_id=purchase_type.purchase_type_id','left');
//                    $this->db->where($where);  
//                    $this->db->order_by("purchase_order.purchase_id","ASC");
//               
...
//    }
	...
	public function get_all_purchase_history($where=array()){
			...
			$this->db->select("purchase_order.purchase_id,
                    ...
                    if($previlige == FALSE)
                    {
                        $this->db->where("purchase_order.created_by IN (SELECT user_id FROM user WHERE location_id=".$this->session->userdata('LOCATION_ID').")",NULL,FALSE);
                    }
                    $this->db->order_by("purchase_order.purchase_id","DESC");
                    $result= $this->db->get();
    ...
    public function get_selected_product2($order_details_id,$field,$table,$pi=NULL){
            $this->db->select("purchase_order_details.*,product.product_name,product.product_details_json,product.product_code,product.model_name,product_category.product_category_name,proforma_invoice.indent_number as pindent_number,proforma_invoice.proforma_invoice_id,proforma_invoice.purchase_order_id,proforma_invoice.lc_number",false);
            $this->db->from("purchase_order_details");
            $this->db->join("product",'product.product_id=purchase_order_details.product_id', 'LEFT');
            $this->db->join("product_category",'product.product_category_id = product_category.product_category_id', 'LEFT');
            $this->db->join("proforma_invoice","proforma_invoice.proforma_invoice_id=purchase_order_details.proforma_invoice_id","left");
            $this->db->where("purchase_order_details.purchase_order_id ",$order_details_id,FALSE);
            if($table == 'region')
            {
            ...
            if($pi)
            {
                $this->db->where("proforma_invoice.proforma_invoice_id",$pi);
                $this->db->group_by("purchase_order_details.product_id");
            }
    ...
    public function proforma_invoice_info($order_id=NULL)
    {
        ...
        $this->db->order_by("po.purchase_code DESC");
        return $this->db->get()->result_array();
    }

    

    public function purchase_order_status($order_id)
    {
    ...
    {
        $this->db->select("product.product_group_id,product.region_id,purchase_order_details.purchase_order_id,purchase_order_details.purchase_order_details_id,purchase_order_details.proforma_invoice_id,".$table.".".$field);
        
        $this->db->select("proforma_invoice.indent_number");
        $this->db->from("purchase_order_details");
        $this->db->join("product",'product.product_id=purchase_order_details.product_id', 'LEFT');
        $this->db->join("product_category",'product.product_category_id = product_category.product_category_id', 'LEFT');
        $this->db->join("proforma_invoice",'purchase_order_details.proforma_invoice_id = proforma_invoice.proforma_invoice_id', 'LEFT');
        if($table == "region")
        {
            $this->db->join("region",'region.region_id = product.region_id', 'LEFT');
        }
        ...
        if($order_id)
        {
            $this->db->where("purchase_order_details.purchase_order_id",$order_id,FALSE);
        }
  ...
  function get_good_receive_info($proforma_invoice_id){
    $this->db->select("purchase_good_receive.receive_quantity,purchase_good_receive.recieve_ack_date,p.product_name,p.product_id");
        ...
        $this->db->where("purchase_order_details.proforma_invoice_id",$proforma_invoice_id);
        $result= $this->db->get();
        //debug($this->db->last_query($result),1);
        return $result->result_array();
    ...
    function fob_product_details_sql($proforma_invoice_id,$product_group_id)
    {
        return $this->db->query("SELECT
                    ...
                    product.sdta,
                    region.region_name,
                    (SELECT purchase_product_fob_details.amount FROM purchase_product_fob_details WHERE purchase_product_fob_details.purchase_order_details_id=purchase_order_details.purchase_order_details_id AND purchase_product_fob_details.cost_details LIKE '%Landed Cost%') AS landed_cost
                    FROM
                    purchase_order_details
                    LEFT JOIN product ON purchase_order_details.product_id = product.product_id
	...
	/*
     * To find date-wise product wise recieve packing serial amount
     */
    ...
    public function update_purchase_status($purchase_id,$purchas_type_id){
        ...
        if($total_quantity==$total_recieved){
            /*update status.all serial recievd
             * status 9 for waiting for shipment
             */
...
//        $join = " ";
//        if(!empty($model_id)){
//            $join .= " AND product_specification.model_id = $model_id";
//        }else {
//             $join .= " AND product_specification.model_id = ''";
//        }	
//        $sql = $this->db->query("SELECT
//            specification_type.specification_type_id,
//            specification_type.product_category_id,
//            specification_type.specification_type_name,
//            product_specification.specification_details
//            FROM
//            specification_type
//            LEFT JOIN product_specification ON specification_type.specification_type_id = product_specification.specification_type_id 
//            AND product_specification.product_id = $product_id $join
//            WHERE
//            specification_type.product_category_id = 1");
        $sql = $this->db->query("SELECT
                    specification_type.specification_type_id,
                    specification_type.specification_type_name,
                    product_specification.specification_details
    ...
    public function get_my_approvd_list($u_id){
        ...
        $this->db->join("user u",'u.user_id=dl.approve_by', 'LEFT');
        $this->db->join("user r",'r.user_id=dl.reliever_of', 'LEFT');
        $this->db->like("dl.ref_no","PO","RIGHT");
